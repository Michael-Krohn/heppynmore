...
parser.add_option("-G", "--source-director", dest="source_director", default='./',
		    help="source directory with the redirector")
parser.add_option("-E", "--end-of-file", dest="file_end", default='.root',
		    help="end of files to hadd")
...
def hadd_files(target_file,target_dir,source_dir,source_director,file_end):
	...
	for __tuple_entry in __tuple_dir_and_files:
		...
		logging.debug(__number_of_files)
		    ...
		    for __file in __tuple_entry[2]:
    		        __list_of_files.extend(['%s/%s' %(source_director, __file)]) # __tuple_entry[0] is the full path of the files
    	# check if these are root files and store the information
    	__skimmed_list_of_files = [el for el in __list_of_files if (file_end in el)]
...
if __name__ == "__main__":
	hadd_files(opts.target_file,opts.target_dir,opts.source_dir,opts.source_director,opts.file_end)
	sys.exit(0)
